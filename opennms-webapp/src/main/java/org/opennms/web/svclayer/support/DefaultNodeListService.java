 ...
 */
public class DefaultNodeListService implements NodeListService,
		InitializingBean {
	...
	public NodeListModel createNodeList(NodeListCommand command) {
		...
		/*
		 * All search queries can be done solely with criteria, so we build a
		 * common criteria object with common restrictions and sort options.
		 * Each specific search query adds its own crtieria restrictions (if
		 * any).
		 * 
		 * A set of booleans is maintained for aliases that might be added in
		 * muliple places to ensure we don't add the same alias multiple times.
		 */
	...
	private void addCriteriaForCommand(OnmsCriteria criteria,
		...
		} else if (command.hasSnmpParm() && command.hasSnmpParmValue()
				&& command.hasSnmpParmMatchType()) {
			addCriteriaForSnmpParm(criteria, command.getSnmpParm(),
					command.getSnmpParmValue(), command.getSnmpParmMatchType());
		} else if (command.hasCategory1() && command.hasCategory2()) {
			addCriteriaForCategories(criteria, command.getCategory1(),
					command.getCategory2());
		} else if (command.hasCategory1()) {
			addCriteriaForCategories(criteria, command.getCategory1());
		} else if (command.hasStatusViewName() && command.hasStatusSite()
				&& command.hasStatusRowLabel()) {
			addCriteriaForSiteStatusView(criteria, command.getStatusViewName(),
					command.getStatusSite(), command.getStatusRowLabel());
	...
	private void addCriteriaForSnmpParm(OnmsCriteria criteria, String snmpParm,
			String snmpParmValue, String snmpParmMatchType) {
		...
		if (snmpParmMatchType.equals("contains")) {
			criteria.add(Restrictions.ilike("snmpInterface.".concat(snmpParm),
					snmpParmValue, MatchMode.ANYWHERE));
		} else if (snmpParmMatchType.equals("equals")) {
			snmpParmValue = snmpParmValue.toLowerCase();
			criteria.add(Restrictions
					.sqlRestriction("{alias}.nodeid in (select nodeid from snmpinterface where snmpcollect != 'D' and lower(snmp"
	...
	private void addCriteriaForCurrentOutages(OnmsCriteria criteria) {
		/*
		 * This doesn't work properly if ipInterfaces and/or monitoredServices
		 * have other restrictions. If we are matching on service ID = 7, but
		 * service ID = 1 has an outage, the node won't show up.
		 */
		/*
		...
		criteria.add(Restrictions
				.sqlRestriction("{alias}.nodeId in (select o.nodeId from outages o where o.ifregainedservice is null and o.suppresstime is null or o.suppresstime < now())"));
	}

	private void addCriteriaForNodename(OnmsCriteria criteria, String nodeName) {
		criteria.add(Restrictions.ilike("node.label", nodeName,
				MatchMode.ANYWHERE));
	}
	...
	private void addCriteriaForForeignSource(OnmsCriteria criteria,
			String foreignSource) {
		criteria.add(Restrictions.ilike("node.foreignSource", foreignSource,
				MatchMode.ANYWHERE));
	}

	private void addCriteriaForIpLike(OnmsCriteria criteria, String iplike) {
		OnmsCriteria ipInterface = criteria.createCriteria("node.ipInterfaces",
				"ipInterface");
	...
	private void addCriteriaForService(OnmsCriteria criteria, int serviceId) {
		...
		criteria.createAlias("node.ipInterfaces.monitoredServices",
				"monitoredService");
		criteria.createAlias("node.ipInterfaces.monitoredServices.serviceType",
				"serviceType");
	...
	private void addCriteriaForMaclike(OnmsCriteria criteria, String macLike) {
		...
		criteria.createAlias("node.arpInterfaces", "arpInterface",
				CriteriaSpecification.LEFT_JOIN);
		Disjunction physAddrDisjunction = Restrictions.disjunction();
		physAddrDisjunction.add(Restrictions.ilike("snmpInterface.physAddr",
				macLikeStripped, MatchMode.ANYWHERE));
		physAddrDisjunction.add(Restrictions.ilike("arpInterface.physAddr",
				macLikeStripped, MatchMode.ANYWHERE));
		...
		// type
		/*
		 * String propertyName = "nodeId"; String value = "%" + macLikeStripped
		 * + "%";
		 * 
		 ...
		 * value, new StringType())); criteria.add(physAddrDisjuction);
		 */
	}
	 ...
	 * categories) { Assert.notNull(categories,
	 * "categories argument must not be null"); Assert.isTrue(categories.length
	 * >= 1, "categories must have at least one set of categories");
	 * 
	 * for (String[] categoryStrings : categories) { for (String categoryString
	 * : categoryStrings) { OnmsCategory category =
	 * m_categoryDao.findByName(categoryString); if (category == null) { throw
	 * new IllegalArgumentException("Could not find category for name '" +
	 * categoryString + "'"); } } }
	 * 
	 * int categoryCount = 0; for (String[] categoryStrings : categories) {
	...
	private void addCriteriaForCategories(OnmsCriteria criteria,
			String[]... categories) {
		Assert.notNull(criteria, "criteria argument must not be null");

		for (Criterion criterion : m_categoryDao
				.getCriterionForCategorySetsUnion(categories)) {
			criteria.add(criterion);
		}
	}

	private void addCriteriaForSiteStatusView(OnmsCriteria criteria,
			String statusViewName, String statusSite, String rowLabel) {
		/*
		 * View view = m_siteStatusViewConfigDao.getView(statusViewName); RowDef
		 * rowDef = getRowDef(view, rowLabel); Set<String> categoryNames =
		 * getCategoryNamesForRowDef(rowDef);
		 * 
		 * addCriteriaForCategories(criteria, categoryNames.toArray(new
		 * String[categoryNames.size()]));
		 * 
		 * String sql = "{alias}.nodeId in (select nodeId from assets where " +
		 * view.getColumnName() + " = ?)";
		 * criteria.add(Restrictions.sqlRestriction(sql, statusSite, new
		 * StringType()));
		 */
	}

	/*
	 * private RowDef getRowDef(View view, String rowLabel) { Rows rows =
	 * view.getRows(); Collection<RowDef> rowDefs = rows.getRowDefCollection();
	 * for (RowDef rowDef : rowDefs) { if (rowDef.getLabel().equals(rowLabel)) {
	 * return rowDef; } }
	 * 
	 * throw new
	 * DataRetrievalFailureException("Unable to locate row: "+rowLabel+
	 * " for status view: "+view.getName()); }
	 */
	 ...
	 * 
	 * List<Category> cats = rowDef.getCategoryCollection(); for (Category cat :
	 * cats) { categories.add(cat.getName()); } return categories; }
	 */

	private NodeListModel createModelForNodes(NodeListCommand command,
		...
		for (OnmsNode node : onmsNodes) {
			...
			if (command.getListInterfaces()) {
				if (command.hasSnmpParm()
						&& command.getSnmpParmMatchType().equals("contains")) {
					String parmValueMatchString = (".*"
							+ command.getSnmpParmValue().toLowerCase()
									.replaceAll("([\\W])", "\\\\$0")
									.replaceAll("\\\\%", ".*")
									.replaceAll("_", ".") + ".*");
					if (command.getSnmpParm().equals("ifAlias")) {
						for (OnmsSnmpInterface snmpIntf : node
								.getSnmpInterfaces()) {
							if (snmpIntf != null
									&& !"D".equals(snmpIntf.getCollect())
									&& snmpIntf.getIfAlias() != null
									&& snmpIntf.getIfAlias().toLowerCase()
											.matches(parmValueMatchString)) {
						...
						for (OnmsSnmpInterface snmpIntf : node
							...
							if (snmpIntf != null
									&& !"D".equals(snmpIntf.getCollect())
									&& snmpIntf.getIfName() != null
									&& snmpIntf.getIfName().toLowerCase()
											.matches(parmValueMatchString)) {
								displaySnmpInterfaces.add(snmpIntf);
							}
						}
						...
						for (OnmsSnmpInterface snmpIntf : node
							...
							if (snmpIntf != null
									&& !"D".equals(snmpIntf.getCollect())
									&& snmpIntf.getIfDescr() != null
									&& snmpIntf.getIfDescr().toLowerCase()
											.matches(parmValueMatchString)) {
								displaySnmpInterfaces.add(snmpIntf);
							}
						}
					}
				} else if (command.hasSnmpParm()
						&& command.getSnmpParmMatchType().equals("equals")) {
					if (command.getSnmpParm().equals("ifAlias")) {
						for (OnmsSnmpInterface snmpIntf : node
								.getSnmpInterfaces()) {
							if (snmpIntf != null
									&& !"D".equals(snmpIntf.getCollect())
									&& snmpIntf.getIfAlias() != null
									&& snmpIntf.getIfAlias().equalsIgnoreCase(
											command.getSnmpParmValue())) {
								displaySnmpInterfaces.add(snmpIntf);
							}
						}
					} else if (command.getSnmpParm().equals("ifName")) {
						for (OnmsSnmpInterface snmpIntf : node
							...
							if (snmpIntf != null
									&& !"D".equals(snmpIntf.getCollect())
									&& snmpIntf.getIfName() != null
									&& snmpIntf.getIfName().equalsIgnoreCase(
											command.getSnmpParmValue())) {
								displaySnmpInterfaces.add(snmpIntf);
							}
						}
						...
						for (OnmsSnmpInterface snmpIntf : node
							...
							if (snmpIntf != null
									&& !"D".equals(snmpIntf.getCollect())
									&& snmpIntf.getIfDescr() != null
									&& snmpIntf.getIfDescr().equalsIgnoreCase(
											command.getSnmpParmValue())) {
								displaySnmpInterfaces.add(snmpIntf);
							}
						}
					...
					String macLikeStripped = command.getMaclike().toLowerCase()
							.replaceAll("[:-]", "");
					for (OnmsIpInterface intf : node.getIpInterfaces()) {
						if (intf.getSnmpInterface() != null
								&& intf.getSnmpInterface().getPhysAddr() != null
								&& intf.getSnmpInterface().getPhysAddr()
										.toLowerCase()
										.contains(macLikeStripped)) {
							displayInterfaces.add(intf);
						}
					}
					for (OnmsSnmpInterface snmpIntf : node.getSnmpInterfaces()) {
						if (snmpIntf.getPhysAddr() != null
								&& !"D".equals(snmpIntf.getCollect())
								&& snmpIntf.getPhysAddr().toLowerCase()
										.contains(macLikeStripped)) {
							OnmsIpInterface intf = snmpIntf
					...
					for (OnmsArpInterface aint : node.getArpInterfaces()) {
						if (aint.getPhysAddr() != null
								&& aint.getPhysAddr().toLowerCase()
										.contains(macLikeStripped)) {
							OnmsIpInterface intf = node
									.getIpInterfaceByIpAddress(aint
											.getIpAddress());
							if ((intf == null
									|| intf.getSnmpInterface() == null
									|| intf.getSnmpInterface().getPhysAddr() == null || !intf
									.getSnmpInterface().getPhysAddr()
									.equalsIgnoreCase(aint.getPhysAddr()))) {
					...
					for (OnmsIpInterface intf : node.getIpInterfaces()) {
						if (!"D".equals(intf.getIsManaged())
								&& !"0.0.0.0".equals(InetAddressUtils.str(intf
										.getIpAddress()))) {
							displayInterfaces.add(intf);
						}
					}
	 ...
	 * 
	 * @param categoryDao
	 *            a {@link org.opennms.netmgt.dao.CategoryDao} object.
	 */
	 ...
	 *            a {@link org.opennms.netmgt.dao.SiteStatusViewConfigDao}
	 *            object.
	 */
	public void setSiteStatusViewConfigDao(
			SiteStatusViewConfigDao siteStatusViewConfigDao) {
	...
	public void afterPropertiesSet() throws Exception {
		...
		Assert.state(m_categoryDao != null,
				"categoryDao property cannot be null");
		Assert.state(m_siteStatusViewConfigDao != null,
				"siteStatusViewConfigDao property cannot be null");
	}

	public static class IpInterfaceComparator implements
			Comparator<OnmsIpInterface>, Serializable {
		/**
         * 
         */
		...
		public int compare(final OnmsIpInterface o1, final OnmsIpInterface o2) {
			...
			if (!"0.0.0.0".equals(o1ip) && !"0.0.0.0".equals(o2ip)) {
				return new ByteArrayComparator().compare(
						InetAddressUtils.toIpAddrBytes(o1ip),
			...
			if (o1.getSnmpInterface().getIfName() == null
					|| o2.getSnmpInterface().getIfName() == null) {
				if (o1.getSnmpInterface().getIfName() != null) {
					return -1;
				} else if (o2.getSnmpInterface().getIfName() != null) {
					return 1;
				}
			} else if ((diff = o1.getSnmpInterface().getIfName()
					.compareTo(o2.getSnmpInterface().getIfName())) != 0) {
				return diff;
			}

			// Sort by ifDescr
			if (o1.getSnmpInterface().getIfDescr() == null
					|| o2.getSnmpInterface().getIfDescr() == null) {
				if (o1.getSnmpInterface().getIfDescr() != null) {
					return -1;
				} else if (o2.getSnmpInterface().getIfDescr() != null) {
					return 1;
				}
			} else if ((diff = o1.getSnmpInterface().getIfDescr()
					.compareTo(o2.getSnmpInterface().getIfDescr())) != 0) {
				return diff;
			}

			// Sort by ifIndex
			if ((diff = o1.getSnmpInterface().getIfIndex()
					.compareTo(o2.getSnmpInterface().getIfIndex())) != 0) {
				return diff;
			}
	 ...
	 * 
	 */
	public static class SnmpInterfaceComparator implements
			Comparator<OnmsSnmpInterface>, Serializable {
		/**
         * 
         */
	...
	public static class ArpInterfaceComparator implements
			Comparator<OnmsArpInterface>, Serializable {
		...
		public int compare(OnmsArpInterface o1, OnmsArpInterface o2) {
			...
			if (!"0.0.0.0".equals(o1.getIpAddress())
				...
				return new ByteArrayComparator().compare(
						InetAddressUtils.toIpAddrBytes(o1.getIpAddress()),
	...
	@Override
	public List<Asset> getNodeList() {
		List<OnmsAssetRecord> records = assetRecordDao.findAll();
		List<Asset> assets = new ArrayList<Asset>();
		for (OnmsAssetRecord record : records) {
			Asset asset = new Asset();
			OnmsNode node = record.getNode();
			if(node == null){
				asset.setNodeId(record.getId());
			}else{
				asset.setNodeId(node.getId());
				asset.setLabel(node.getLabel());
			}
			asset.setCategory(record.getCategory());
			asset.setVendor(record.getVendor());
			assets.add(asset);
		}
		return assets;
	}
	...
	@Override
	public int createAssetRecord(OnmsAssetRecord record) {
		assetRecordDao.saveOrUpdate(record);
		return record.getId();
	}
}
