...
<%@page language="java"
	...
	import="
		...
		org.opennms.web.rest.support.TimeSeries,
...
<head>
  ...
  <c:if test="${param.nostyles != 'true' }">
   ...
   <c:if test="${param.useStyle == 'true'}">
      <link rel="stylesheet" type="text/css" href="<%= baseHref %>css/style.css" />
  </c:if>
...
<c:if test="${((param.nonavbar != 'true') && (!empty pageContext.request.remoteUser)) && param.nobreadcrumbs != 'true'}">
  <ol class="breadcrumb">
    <li><a href="<%= baseHref %>index.jsp">主页</a></li>
