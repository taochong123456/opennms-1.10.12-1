...
public class HttpCollector implements ServiceCollector {
	...
	public CollectionSet collect(CollectionAgent agent, EventProxy eproxy,
		...
		HttpCollectionSet collectionSet = new HttpCollectionSet(agent,
				parameters);
	...
	protected class HttpCollectionSet implements CollectionSet {
		...
		public void collect() {
			String collectionName = ParameterMap.getKeyedString(m_parameters,
					"collection", null);
			if (collectionName == null) {
				// Look for the old configuration style:
				collectionName = ParameterMap.getKeyedString(m_parameters,
						"http-collection", null);
			}
			if (collectionName == null) {
			...
			HttpCollection collection = HttpCollectionConfigFactory
					.getInstance().getHttpCollection(collectionName);
			...
			for (Uri uriDef : uriDefs) {
				m_uriDef = uriDef;
				HttpCollectionResource collectionResource = new HttpCollectionResource(
						m_agent, uriDef, collectionName);
				try {
		...
		public int getPort() { // This method has been created to deal with
			...
			if (port == 80 && m_parameters.containsKey("port")) {
				try {
					port = Integer
							.parseInt(m_parameters.get("port").toString());
					log().debug(
							"getPort: using service provided HTTP port " + port);
				} catch (Exception e) {
	 ...
	 * 
	 * Couple of notes to make the implementation of this client library less
	 * obtuse:
	 * 
	 * - HostConfiguration class is not created here because the library builds
	 * it when a URI is defined.
	 * 
	 * @param collectionSet
	...
	public void doCollection(final HttpCollectionSet collectionSet,
		...
		try {
			...
			if ("https".equals(collectionSet.getUriDef().getUrl().getScheme())) {
				final SchemeRegistry registry = client.getConnectionManager()
						.getSchemeRegistry();
				...
				final SSLSocketFactory factory = new SSLSocketFactory(
						SSLContext
								.getInstance(EmptyKeyRelaxedTrustSSLContext.ALGORITHM),
						SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);

				final Scheme lenient = new Scheme(https.getName(),
						https.getDefaultPort(), factory);
			...
			String matches = collectionSet.getUriDef().getUrl().getMatches();
			String path = collectionSet.getUriDef().getUrl().getPath();
			String address = collectionSet.getAgent().getAddress().toString()
					.substring(1);
			if ("all".equals(matches)) {
				URLDataCollector urlDataCollector = new URLDataCollector();
				Properties props = new Properties();
				int port = collectionSet.getPort();
				props.put("HostName", address);
				props.put("Port", port + "");
				String hostportURL = hostURL(props);
				setProperties(hostportURL, path, props);
				HashMap results = urlDataCollector.startDatacollection(props);
				String htmlresponse = (String) results.get("htmlresponse");

				List<HttpCollectionAttribute> attributes = processJsonResponse(
						Locale.ENGLISH, htmlresponse, collectionSet,
						collectionResource);
				if (attributes.isEmpty()) {
					log().warn(
							"doCollection: no attributes defined by the response: ");
	...
	private void setProperties(String hosturl, String restApiURL,
		...
		config.put("mn", "es");
		config.put("a", esURI);
		config.put("hn", "esHeader");
		config.put("hv", "esValue");
		config.put("webContent", "yes");
		config.put("returnRespStr", "true");
	}
	...
	private List<HttpCollectionAttribute> processJsonResponse(Locale english,
			String htmlresponse, HttpCollectionSet collectionSet,
			HttpCollectionResource collectionResource) {
		List<HttpCollectionAttribute> butes = new LinkedList<HttpCollectionAttribute>();
		if (htmlresponse != null) {
			Map<String, Object> keyToValue = JsonTransform
					.stringToMap(htmlresponse);
			...
			for (final Attrib attribDef : attribDefs) {
				...
				String attrValue = keyToValue.get(attribDef.getAlias()) + "";
				HttpCollectionAttribute bute = new HttpCollectionAttribute(
						collectionResource, new HttpCollectionAttributeType(
								attribDef, groupType), attribDef.getAlias(),
						type, attrValue);
				log().debug(
						"processResponse: adding found string attribute: "
								+ bute);
				butes.add(bute);
			}
		}
	...
	class HttpCollectionAttribute extends AbstractCollectionAttribute implements
			AttributeDefinition {
		String m_alias;
		String m_type;
		Object m_value;
		...
		HttpCollectionAttribute(HttpCollectionResource resource,
				HttpCollectionAttributeType attribType, String alias,
	...
	private List<HttpCollectionAttribute> processResponse(
			final Locale responseLocale, final String responseBodyAsString,
			final HttpCollectionSet collectionSet,
			HttpCollectionResource collectionResource) {
		log().debug("processResponse:");
		log().debug("responseBody = " + responseBodyAsString);
		log().debug(
				"getmatches = "
						+ collectionSet.getUriDef().getUrl().getMatches());
		List<HttpCollectionAttribute> butes = new LinkedList<HttpCollectionAttribute>();
		int flags = 0;
		...
		Pattern p = Pattern.compile(collectionSet.getUriDef().getUrl()
				.getMatches(), flags);
		...
		if (matches) {
			log().debug(
					"processResponse: found matching attributes: " + matches);
			final List<Attrib> attribDefs = collectionSet.getUriDef()
					.getAttributes().getAttribCollection();
			final AttributeGroupType groupType = new AttributeGroupType(
					collectionSet.getUriDef().getName(), "all");
			...
			for (final Attrib attribDef : attribDefs) {
				...
				try {
					...
					log().error(
							"Regex statement: "
									+ collectionSet.getUriDef().getUrl()
											.getMatches());
					continue;
				}

				if (!type
						.matches("^([Oo](ctet|CTET)[Ss](tring|TRING))|([Ss](tring|TRING))$")) {
					Number num = null;
					for (final Locale locale : locales) {
						try {
							num = NumberFormat.getNumberInstance(locale).parse(
									value);
							log().debug(
									"processResponse: found a parsable number with locale \""
											+ locale + "\".");
							break;
							...
							log().error(
									"attribute "
											+ attribDef.getAlias()
											+ " failed to match a parsable number with locale \""
											+ locale + "\"! Matched \"" + value
											+ "\" instead.");
						}
					}
					...
					if (num == null) {
						log().error(
								"processResponse: gave up attempting to parse numeric value, skipping group "
										+ attribDef.getMatchGroup());
						continue;
					}
					...
					final HttpCollectionAttribute bute = new HttpCollectionAttribute(
							collectionResource,
							new HttpCollectionAttributeType(attribDef,
									groupType), attribDef.getAlias(), type, num);
					log().debug(
							"processResponse: adding found numeric attribute: "
									+ bute);
					butes.add(bute);
				} else {
					HttpCollectionAttribute bute = new HttpCollectionAttribute(
							collectionResource,
							new HttpCollectionAttributeType(attribDef,
									groupType), attribDef.getAlias(), type,
							value);
					log().debug(
							"processResponse: adding found string attribute: "
									+ bute);
	...
	private void persistResponse(final HttpCollectionSet collectionSet,
			final HttpCollectionResource collectionResource,
			final HttpClient client, final HttpResponse response)
			throws IOException {
		...
		if (responseString != null && !"".equals(responseString)) {
			...
			if (headers != null) {
				log().debug(
						"doCollection: Trying to devise response's locale from Content-Language header.");
				if (headers.length == 1) {
					if (headers[0].getValue().split(",").length == 1) {
						final String[] values = headers[0].getValue()
								.split("-");
						log().debug(
								"doCollection: Found one Content-Language header with value: "
										+ headers[0].getValue());
						...
						default:
							log().warn(
									"doCollection: Ignoring Content-Language header with value "
											+ headers[0].getValue()
											+ ". No support for more than 1 language subtag!");
						}
					} else {
						log().warn(
								"doCollection: Multiple languages specified. That doesn't make sense. Ignoring...");
					}
				} else {
					log().warn(
							"doCollection: More than 1 Content-Language headers received. Ignoring them!");
				}
			}

			List<HttpCollectionAttribute> attributes = processResponse(
					responseLocale, responseString, collectionSet,
					collectionResource);
	...
	private static void buildCredentials(final HttpCollectionSet collectionSet,
			final DefaultHttpClient client, final HttpUriRequest method) {
		if (collectionSet.getUriDef().getUrl().getUserInfo() != null) {
			...
			if (streetCred.length == 2) {
				...
				log().warn(
						"Illegal value found for username/password HTTP credentials: "
								+ userInfo);
			}
		}
	}
	...
	private static HttpParams buildParams(final HttpCollectionSet collectionSet) {
		...
		params.setParameter(CoreProtocolPNames.PROTOCOL_VERSION,
				computeVersion(collectionSet.getUriDef()));
		params.setIntParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, Integer
				.parseInt(ParameterMap.getKeyedString(
						collectionSet.getParameters(), "timeout",
						DEFAULT_SO_TIMEOUT)));
		...
		String virtualHost = collectionSet.getUriDef().getUrl()
				.getVirtualHost();
		if (virtualHost != null) {
			params.setParameter(ClientPNames.VIRTUAL_HOST, new HttpHost(
					virtualHost, collectionSet.getPort()));
		}
	...
	private static String determineUserAgent(
			final HttpCollectionSet collectionSet, final HttpParams params) {
		String userAgent = collectionSet.getUriDef().getUrl().getUserAgent();
		return (String) (userAgent == null ? params
				.getParameter(CoreProtocolPNames.USER_AGENT) : userAgent);
	}

	private static HttpVersion computeVersion(final Uri uri) {
		return new HttpVersion(Integer.parseInt(uri.getUrl().getHttpVersion()
				.substring(0, 1)), Integer.parseInt(uri.getUrl()
				.getHttpVersion().substring(2)));
	}

	private static HttpUriRequest buildHttpMethod(
			final HttpCollectionSet collectionSet) throws URISyntaxException {
	...
	private static HttpPost buildPostMethod(final URI uri,
			final HttpCollectionSet collectionSet) {
	...
	private static HttpGet buildGetMethod(final URI uri,
			final HttpCollectionSet collectionSet) {
		...
		try {
			...
			uriWithQueryString = URIUtils.createURI(uri.getScheme(),
	...
	private static List<NameValuePair> buildRequestParameters(
			final HttpCollectionSet collectionSet) {
		List<NameValuePair> retval = new ArrayList<NameValuePair>();
		if (collectionSet.getUriDef().getUrl().getParameters() == null)
			return retval;
		List<Parameter> parameters = collectionSet.getUriDef().getUrl()
				.getParameters().getParameterCollection();
	...
	private static URI buildUri(final HttpCollectionSet collectionSet)
			throws URISyntaxException {
		HashMap<String, String> substitutions = new HashMap<String, String>();
		substitutions
				.put("ipaddr", InetAddressUtils.str(collectionSet.getAgent()
						.getInetAddress()));
		substitutions.put("nodeid",
				Integer.toString(collectionSet.getAgent().getNodeId()));

		return URIUtils.createURI(
				collectionSet.getUriDef().getUrl().getScheme(),
				substituteKeywords(substitutions, collectionSet.getUriDef()
						.getUrl().getHost(), "getHost"),
				collectionSet.getPort(),
				substituteKeywords(substitutions, collectionSet.getUriDef()
						.getUrl().getPath(), "getURL"),
				substituteKeywords(substitutions, collectionSet.getUriDef()
						.getUrl().getQuery(), "getQuery"),
				substituteKeywords(substitutions, collectionSet.getUriDef()
						.getUrl().getFragment(), "getFragment"));
	}

	private static String substituteKeywords(
			final HashMap<String, String> substitutions,
			final String urlFragment, final String desc) {
		String newFragment = urlFragment;
		if (newFragment != null) {
			for (String key : substitutions.keySet()) {
				newFragment = newFragment.replaceAll("\\$\\{" + key + "\\}",
						substitutions.get(key));
			}
			if (log().isDebugEnabled()
					&& newFragment.compareTo(urlFragment) != 0) {
				log().debug(
						"doSubs: " + desc + " substituted as \"" + newFragment
								+ "\"");
			}
		}
		return newFragment;
	...
	private static void initHttpCollectionConfig() {
		try {
			log().debug(
					"initialize: Initializing collector: "
							+ HttpCollector.class.getSimpleName());
	...
	private static void initializeRrdRepository()
			throws CollectionInitializationException {
		log().debug(
				"initializeRrdRepository: Initializing RRD repo from HttpCollector...");
		initializeRrdDirs();
	}
	...
	private static void initDatabaseConnectionFactory() {
		try {
			...
			log().fatal(
					"initDatabaseConnectionFactory: IOException getting database connection",
					e);
			throw new UndeclaredThrowableException(e);
		} catch (MarshalException e) {
			log().fatal(
					"initDatabaseConnectionFactory: Marshall Exception getting database connection",
					e);
			throw new UndeclaredThrowableException(e);
		} catch (ValidationException e) {
			log().fatal(
			...
			log().fatal(
					"initDatabaseConnectionFactory: Failed getting connection to the database.",
					e);
			throw new UndeclaredThrowableException(e);
			...
			log().fatal(
					"initDatabaseConnectionFactory: Failed loading database driver.",
					e);
			throw new UndeclaredThrowableException(e);
		}
	}
	...
	public void initialize(CollectionAgent agent, Map<String, Object> parameters) {
		log().debug(
				"initialize: Initializing HTTP collection for agent: " + agent);
		...
		if (scheduledAddress != null) {
			log().info(
					"initialize: Not scheduling interface for collection: "
							+ scheduledAddress);
			...
			log().info(
					"initialize: Scheduling interface for collection: "
							+ scheduledAddress);
	...
	class HttpCollectionResource implements CollectionResource {
		...
		HttpCollectionResource(CollectionAgent agent, Uri uriDef,
			...
			m_attribGroup = new AttributeGroup(this, new AttributeGroupType(
					uriDef.getName(), "all"));
		}
		...
		public File getResourceDir(RrdRepository repository) {
			return new File(repository.getRrdBaseDir(),
					Integer.toString(m_agent.getNodeId()) + File.separator
							+ serviceName);
		}
	...
	class HttpMultiInstanceCollectionResource extends HttpCollectionResource {
		private String m_inst;
		private String m_name;

		HttpMultiInstanceCollectionResource(CollectionAgent agent, Uri uriDef,
				String instance, String name) {
			super(agent, uriDef, null);
		...
		public File getResourceDir(RrdRepository repository) {
			...
			final File nodeDir = new File(rrdBaseDir, String.valueOf(m_agent
					.getNodeId()));
			final File typeDir = new File(nodeDir, m_name);
			final File instDir = new File(typeDir, m_inst
	...
	class HttpCollectionAttributeType implements CollectionAttributeType {
		...
		protected HttpCollectionAttributeType(Attrib attribute,
	...
	public RrdRepository getRrdRepository(String collectionName) {
		return HttpCollectionConfigFactory.getInstance().getRrdRepository(
				collectionName);
	}
