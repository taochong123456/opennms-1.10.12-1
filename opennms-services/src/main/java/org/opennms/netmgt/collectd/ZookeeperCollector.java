 ...
 * zookeeper collector
 * **/
public class ZookeeperCollector extends JMXCollector {
	private static final String SERVICENAME = "Zookeeper";
	public ZookeeperCollector(){
		super();
        setServiceName(SERVICENAME);
	}
	public CollectionSet collect(CollectionAgent agent, EventProxy eproxy,
			Map<String, Object> map) {
		ZookeeperDataCollector zookeeperCollector = new ZookeeperDataCollector();
		...
		try {
			...
			zookeeperCollector.startDatacollection(connection);
			Map<String, String> basicInfos = zookeeperCollector.getBasicInfo();
			...
			Map<String, List<String>> clusterTable = zookeeperCollector
					.getClusterTableAttributeValues();
			for (Iterator iterator = clusterTable.keySet().iterator(); iterator
					.hasNext();) {
				String replicaName = (String) iterator.next();
                List<String> replis = clusterTable.get(replicaName);
				AttributeGroupType attribGroupType = new AttributeGroupType(
						"replicaName", "all");
				JMXDataSource dataSource = new JMXDataSource();
				dataSource.setName("quorumAddress-"+replis.get(2).split(":")[0]);
				dataSource.setType("String");
				JMXCollectionAttributeType jmxCollectionAttributeType = new JMXCollectionAttributeType(
						dataSource, null, null, attribGroupType);
				m_collectionResource.setAttributeValue(
						jmxCollectionAttributeType, replis.get(2));
				collectionResources.add(m_collectionResource);
			}
	...
	public Jsr160ConnectionWrapper getMBeanServerConnection(
			Map<String, Object> parameterMap, InetAddress address) {
		return Jsr160ConnectionFactory.getMBeanServerConnection(parameterMap,
				address);
	}
