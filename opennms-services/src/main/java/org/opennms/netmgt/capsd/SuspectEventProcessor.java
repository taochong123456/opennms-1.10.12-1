...
import org.opennms.netmgt.dao.DbIfServiceEntry;
import org.opennms.netmgt.dao.DbIpInterfaceEntry;
import org.opennms.netmgt.dao.DbNodeEntry;
import org.opennms.netmgt.dao.DbSnmpInterfaceEntry;
...
final class SuspectEventProcessor implements Runnable {
	 ...
	 */
	SuspectEventProcessor(CapsdDbSyncer capsdDbSyncer,
			PluginManager pluginManager, String ifAddress, int nodeId) {
		Assert.notNull(capsdDbSyncer,
				"The capsdDbSyncer argument cannot be null");
		Assert.notNull(pluginManager,
				"The pluginManager argument cannot be null");
	...
	private DbNodeEntry getExistingNodeEntry(java.sql.Connection dbc,
		...
		if (log().isDebugEnabled())
			log().debug(
					"getExistingNodeEntry: checking for current target: "
							+ collector.getTarget());
		...
		for (IfTableEntry ifEntry : ifTable) {
			...
			if (log().isDebugEnabled())
				log().debug(
						"getExistingNodeEntry: number of interfaces retrieved for ifIndex "
			...
			for (InetAddress ipAddress : ipAddrs) {
				...
				if (ipAddress == null || str(ipAddress).equals("0.0.0.0")
						|| ipAddress.isLoopbackAddress())
					continue;

				if (firstAddress) {
					sqlBuffer.append("ipaddr='").append(str(ipAddress))
							.append("'");
					firstAddress = false;
				} else
					sqlBuffer.append(" OR ipaddr='").append(str(ipAddress))
							.append("'");
		...
		if (log().isDebugEnabled())
			log().debug(
					"getExistingNodeEntry: issuing SQL command: "
							+ sqlBuffer.toString());
		...
		if (ipaddrsOfNewNode.containsAll(ipaddrsOfOldNode)) {
			if (log().isDebugEnabled())
				log().debug(
						"getExistingNodeEntry: found one of the addrs under existing node: "
								+ nodeID);
	 ...
	 */
	private String getDuplicateIpaddress(List<String> ipListA,
			List<String> ipListB) {
		...
		while (iter.hasNext()) {
			...
			if (ipListB.contains(ipaddr)) {
				if (log().isDebugEnabled())
					log().debug(
							"getDuplicateIpaddress: get duplicate ip address: "
									+ ipaddr);
				break;
			} else
				ipaddr = null;
	...
	private DbNodeEntry createNode(Connection dbc, InetAddress ifaddr,
			IfCollector collector, int nodeId) throws SQLException {
		...
		try {
			primaryIf = InetAddress.getByName(m_suspectIf);
		...
		//
		DbNodeEntry entryNode =new DbNodeEntry(nodeId);
		...
		if (snmpc != null) {
			if (snmpc.hasSystemGroup()) {
				...
				if (sysObjectId != null)
					...
					log().warn(
							"SuspectEventProcessor: " + str(ifaddr)
									+ " has NO sysObjectId!!!!");
				...
				if (str != null && str.length() > 0) {
					...
					if (entryNode.getLabelSource() == DbNodeEntry.LABEL_SOURCE_ADDRESS) {
						...
						entryNode
								.setLabelSource(DbNodeEntry.LABEL_SOURCE_SYSNAME);
					}
				}
	...
	private void addInterfaces(Connection dbc, DbNodeEntry node,
		...
		DbIpInterfaceEntry ipIfEntry = DbIpInterfaceEntry
				.create(nodeId, ifaddr);
		ipIfEntry.setLastPoll(now);
		ipIfEntry.setHostname(ifaddr.getHostName());

		/*
		 * NOTE: (reference internal bug# 201) If the ip is 'managed', it might
		 * still be 'not polled' based on the poller configuration The package
		 * filter evaluation requires that the ip be in the database - at this
		 * point the ip is NOT in db, so insert as active and update afterward
		 * Try to avoid re-evaluating the ip against filters for each service,
		 * try to get the first package here and use that for service evaluation
		 */
		...
		org.opennms.netmgt.config.poller.Package ipPkg = getPackageForNewInterface(
				dbc, ifaddr, ipIfEntry, addrUnmanaged);

		int ifIndex = addSnmpInterfaces(dbc, ifaddr, nodeId, collector,
				ipIfEntry);

		// Add supported protocols
		addSupportedProtocols(node, ifaddr, collector.getSupportedProtocols(),
	...
	private int addSnmpInterfaces(Connection dbc, InetAddress ifaddr,
			...
			throws SQLException {
		boolean addedSnmpInterfaceEntry = addIfTableSnmpInterfaces(dbc, ifaddr,
				nodeId, collector);
		...
		if (ifIndex == CapsdConfig.LAME_SNMP_HOST_IFINDEX
			...
			DbSnmpInterfaceEntry snmpEntry = DbSnmpInterfaceEntry.create(
					nodeId, ifIndex);
			snmpEntry.store(dbc);
		}
		...
		if (log().isDebugEnabled()) {
			log().debug(
					"SuspectEventProcessor: setting ifindex for " + nodeId
							+ "/" + ifaddr + " to " + ifIndex);
		}
	...
	private org.opennms.netmgt.config.poller.Package getPackageForNewInterface(
		...
		if (log().isDebugEnabled()) {
			log().debug(
					"addInterfaces: " + ifaddr + " is to be polled = "
							+ ipToBePolled);
		}
	...
	private int getIfIndexForNewInterface(Connection dbc, InetAddress ifaddr,
		...
		if (snmpc.hasIpAddrTable()
			...
			if (log().isDebugEnabled()) {
				log().debug(
						"SuspectEventProcessor: no valid ifIndex for " + ifaddr
								+ " Assume this is a lame SNMP host");
			}
		}
		ipIfEntry.store(dbc);
	...
	private void getSubInterfacesForNewInterface(Connection dbc,
		...
		addSubIpInterfaces(dbc, node, collector, now, nodeId, cFactory,
	...
	private void addSubIpInterfaces(Connection dbc, DbNodeEntry node,
			...
			PollerConfig pollerCfgFactory, IfSnmpCollector snmpc)
			throws SQLException {
		...
		Map<InetAddress, List<SupportedProtocol>> extraTargets = collector
		...
		for (InetAddress xifaddr : extraTargets.keySet()) {
			 ...
			 * and update afterward. Try to avoid re-evaluating the ip against
			 * filters for each service, try to get the first package here and
			 * use that for service evaluation.
			 */
			...
			if ((xifIndex = snmpc.getIfIndex(xifaddr)) != -1) {
				 ...
				 * here. We should only do it if an snmpInterface entry was
				 * previously created for this ifIndex. It was likely done by
				 * addSnmpInterfaces, but I have't checked to make sure that all
				 * cases are covered. - dj@opennms.org
				 */
				...
				if (!supportsSnmp(extraTargets.get(xifaddr))) {
					log().debug(
							"addInterfaces: Interface doesn't support SNMP. "
				 ...
				 */
				log().debug(
						"addInterfaces: No ifIndex found. " + str(xifaddr)
								+ " set to not eligible");
			}
			...
			if (!xaddrUnmanaged) {
				...
				if (!xipToBePolled) {
					// update ismanaged to 'N' in ipinterface
					xipIfEntry
							.setManagedState(DbIpInterfaceEntry.STATE_NOT_POLLED);
					xipIfEntry.store(dbc);
				}
			}

			// add the supported protocols
			addSupportedProtocols(node, xifaddr, extraTargets.get(xifaddr),
					xaddrUnmanaged, xifIndex, xipPkg);
		}
	}

	private boolean addIfTableSnmpInterfaces(Connection dbc,
			InetAddress ifaddr, int nodeId, IfCollector collector)
			throws SQLException {
		if (!collector.hasSnmpCollection()) {
			return false;
		}
		...
		for (IfTableEntry ifte : snmpc.getIfTable()) {
			...
			final DbSnmpInterfaceEntry snmpEntry = DbSnmpInterfaceEntry.create(
					nodeId, xifIndex);
			...
			if (log().isDebugEnabled() && addrs != null) {
				log().debug(
						"SuspectEventProcessor: " + str(addrs[0])
			...
			IfXTableEntry ifxte = snmpc.hasIfXTable() ? snmpc.getIfXTable()
					.getEntry(xifIndex) : null;
	...
	private void addSupportedProtocols(DbNodeEntry node, InetAddress ifaddr,
			List<SupportedProtocol> protocols, boolean addrUnmanaged,
			int ifIndex, org.opennms.netmgt.config.poller.Package ipPkg)
			throws SQLException {
		...
		for (SupportedProtocol p : protocols) {
			...
			DbIfServiceEntry ifSvcEntry = DbIfServiceEntry.create(
					node.getNodeId(), ifaddr, sid.intValue());
			...
			if (addrUnmanaged)
				...
				if (isServicePolledLocally(str(ifaddr), p.getProtocolName(),
						ipPkg)) {
					ifSvcEntry.setStatus(DbIfServiceEntry.STATUS_ACTIVE);
				} else if (isServicePolled(str(ifaddr), p.getProtocolName(),
						ipPkg)) {
	...
	private boolean isServicePolled(String ifAddr, String svcName,
			org.opennms.netmgt.config.poller.Package ipPkg) {
		boolean svcToBePolled = false;
		if (ipPkg != null) {
			svcToBePolled = PollerConfigFactory.getInstance().isPolled(svcName,
					ipPkg);
			if (!svcToBePolled)
	...
	private boolean isServicePolledLocally(String ifAddr, String svcName,
		...
		if (ipPkg != null && !ipPkg.getRemote()) {
			svcToBePolled = PollerConfigFactory.getInstance().isPolled(svcName,
					ipPkg);
			if (!svcToBePolled)
				svcToBePolled = PollerConfigFactory.getInstance()
						.isPolledLocally(ifAddr, svcName);
		}
		return svcToBePolled;
	}
	...
	static boolean hasIfIndex(InetAddress ipaddr, IfSnmpCollector snmpc) {
		...
		if (log().isDebugEnabled())
			log().debug(
					"hasIfIndex: ipAddress: " + str(ipaddr) + " has ifIndex: "
							+ ifIndex);
		if (ifIndex == -1)
			return false;
		else
	...
	static int getIfType(InetAddress ipaddr, IfSnmpCollector snmpc) {
		...
		if (log().isDebugEnabled())
			log().debug(
					"getIfType: ipAddress: " + str(ipaddr) + " has ifIndex: "
							+ ifIndex + " and ifType: " + ifType);
		return ifType;
	}
	 ...
	 * and is a loopback interface. This is in order to allow a non-127.*.*.*
	 * loopback address to be chosen as the primary SNMP interface.
	 * 
	 * @param collector
	...
	private List<InetAddress> buildLBSnmpAddressList(IfCollector collector) {
		...
		if (collector.getSnmpCollector() == null) {
			if (log().isDebugEnabled())
				log().debug(
						"buildLBSnmpAddressList: no SNMP info for "
								+ collector.getTarget());
			return addresses;
		}
		...
		if (!snmpc.hasIfTable() || !snmpc.hasIpAddrTable()) {
			log().info(
					"buildLBSnmpAddressList: missing SNMP info for "
							+ collector.getTarget());
			return addresses;
		}
		...
		if (supportsSnmp(collector.getSupportedProtocols())
			...
			if (log().isDebugEnabled())
				log().debug(
						"buildLBSnmpAddressList: adding target interface "
								+ str(ipAddr)
		...
		if (collector.hasAdditionalTargets()) {
			...
			for (InetAddress currIf : extraTargets.keySet()) {
				...
				if (supportsSnmp(extraTargets.get(currIf))
					...
					if (log().isDebugEnabled())
						log().debug(
								"buildLBSnmpAddressList: adding subtarget interface "
										+ str(currIf)
										+ " temporarily marked as primary!");
	...
	private List<InetAddress> buildSnmpAddressList(IfCollector collector) {
		...
		if (collector.getSnmpCollector() == null) {
			if (log().isDebugEnabled())
				log().debug(
						"buildSnmpAddressList: no SNMP info for "
								+ collector.getTarget());
			return addresses;
		}
		...
		if (!snmpc.hasIfTable() || !snmpc.hasIpAddrTable()) {
			log().info(
					"buildSnmpAddressList: missing SNMP info for "
							+ collector.getTarget());
			return addresses;
		}
		...
		if (supportsSnmp(collector.getSupportedProtocols())
			...
			if (log().isDebugEnabled())
				log().debug(
						"buildSnmpAddressList: adding target interface "
								+ str(ipAddr)
								+ " temporarily marked as primary!");
			addresses.add(ipAddr);
		}
		...
		if (collector.hasAdditionalTargets()) {
			...
			for (InetAddress currIf : extraTargets.keySet()) {
				...
				if (supportsSnmp(extraTargets.get(currIf))
					...
					if (log().isDebugEnabled())
						log().debug(
								"buildSnmpAddressList: adding subtarget interface "
										+ str(currIf)
										+ " temporarily marked as primary!");
					addresses.add(currIf);
				}
	...
	public void run() {
		...
		if (log().isDebugEnabled())
			log().debug(
					"SuspectEventProcessor: running collection for "
							+ str(ifaddr));

		IfCollector collector = new IfCollector(m_pluginManager, ifaddr, true, m_nodeId);
		collector.run();
		// Update the database
		//
				...
				try {
					...
					if (!m_capsdDbSyncer.isInterfaceInDB(dbc, ifaddr)) {
						...
						if (entryNode == null) {
							...
							//
							entryNode = createNode(dbc, ifaddr, collector,m_nodeId);
						} else {
						...
						// Add interfaces
						//
						addInterfaces(dbc, entryNode, useExistingNode, ifaddr,
								collector);
						...
						//
						boolean strict = true;
						CollectdConfigFactory.getInstance()
						...
						while (iter.hasNext()) {
							...
							if (CollectdConfigFactory.getInstance()
									.isServiceCollectionEnabled(str(addr),
											"SNMP")) {
								
								updatePrimaryInterface("S",entryNode.getNodeId(),addr.getHostAddress(),dbc);
							}
						}
						updatePrimaryInterface("P",entryNode.getNodeId(),m_suspectIf,dbc);
		...
		if (log().isDebugEnabled()) {
			log().debug(
					"sendInterfaceEvents: sending suspect scan completed event for "
							+ str(ifaddr));
			log().debug(
					"SuspectEventProcessor for " + m_suspectIf + " completed.");
		}
		createAndSendSuspectScanCompletedEvent(ifaddr);
	} // end run

	public void updatePrimaryInterface(String isPrimary,int nodeId,String ipaddr,Connection dbc){

		final DBUtils d = new DBUtils(getClass());
		try {
			PreparedStatement stmt = dbc
					.prepareStatement("UPDATE ipInterface SET isSnmpPrimary=? WHERE nodeId=? AND ipAddr=? AND isManaged!='D'");
			d.watch(stmt);
			stmt.setString(1, isPrimary);
			stmt.setInt(2, nodeId);
			stmt.setString(3, ipaddr);

			stmt.executeUpdate();
		} catch (SQLException e) {
			log().error("updatePrimaryInterface");
		} finally {
			d.cleanUp();
		}
	 ...
	 */
	List<InetAddress> getPrimarySnmpInterfaceFromDb(Connection dbc,
			DbNodeEntry node) throws SQLException {
		List<InetAddress> priSnmpAddrs = new ArrayList<InetAddress>();

		log().debug(
				"getPrimarySnmpInterfaceFromDb: retrieving primary SNMP interface(s) from DB for node "
						+ node.getNodeId());
		...
		try {
			PreparedStatement stmt = dbc
					.prepareStatement("SELECT ipAddr FROM ipInterface WHERE nodeId=? AND isSnmpPrimary='P' AND isManaged!='D'");
			d.watch(stmt);
			...
			while (rs.next()) {
				...
				log().debug(
						"getPrimarySnmpInterfaceFromDb: String oldPrimaryAddr = "
								+ oldPrimaryAddr);
				if (oldPrimaryAddr != null) {
					oldPrimarySnmpIf = addr(oldPrimaryAddr);
					log().debug(
							"getPrimarySnmpInterfaceFromDb: old primary SNMP interface is "
									+ oldPrimaryAddr);
	...
	/**
	 * Responsible for setting the Set used to track suspect scans that are
	 * already enqueued for processing. Should be called once by Capsd at
	 * startup.
	 * 
	 * @param queuedSuspectTracker
	 *            a {@link java.util.Set} object.
	 */
	public static synchronized void setQueuedSuspectsTracker(
			Set<String> queuedSuspectTracker) {
		m_queuedSuspectTracker = Collections
				.synchronizedSet(queuedSuspectTracker);
	}
	...
	private void generateSnmpDataCollectionEvents(DbNodeEntry nodeEntry,
		...
		if (oldPrimary == null && newPrimary == null) {
			log().warn(
					"generateSnmpDataCollectionEvents: both old and new primary SNMP interface vars are null!");
		}

		// Sanity check -- should not happen
		else if (oldPrimary != null && newPrimary == null) {
			log().warn(
					"generateSnmpDataCollectionEvents: old primary ("
							+ str(oldPrimary)
		...
		// additional events are required.
			...
			if (log().isDebugEnabled())
				log().debug(
						"generateSnmpDataCollectionEvents: identified "
								+ str(newPrimary)
		...
		//
			...
			if (log().isDebugEnabled()) {
				log().debug(
						"generateSnmpDataCollectionEvents: primary SNMP interface has changed.  Was: "
								+ str(oldPrimary) + " Is: " + str(newPrimary));
			}

			createAndSendPrimarySnmpInterfaceChangedEvent(
					nodeEntry.getNodeId(), newPrimary, oldPrimary);
		}
	...
	private void createAndSendPrimarySnmpInterfaceChangedEvent(int nodeId,
		...
		if (log().isDebugEnabled())
			log().debug(
					"createAndSendPrimarySnmpInterfaceChangedEvent: nodeId: "
							+ nodeId + " oldPrimarySnmpIf: '"
							+ str(oldPrimaryIf) + "' newPrimarySnmpIf: '"
							+ str(newPrimaryIf) + "'");
	...
	private void createAndSendReinitializePrimarySnmpInterfaceEvent(int nodeId,
			InetAddress primarySnmpIf) {
		if (log().isDebugEnabled())
			log().debug(
					"reinitializePrimarySnmpInterface: nodeId: " + nodeId
	 ...
	 */
	private void sendInterfaceEvents(DbNodeEntry node, boolean useExistingNode,
			InetAddress ifaddr, IfCollector collector) {
		// nodeGainedInterface
		//
		if (log().isDebugEnabled())
			log().debug(
					"sendInterfaceEvents: sending node gained interface event for "
							+ str(ifaddr));
		...
		//
		log().debug(
				"sendInterfaceEvents: processing supported services for "
						+ str(ifaddr));
		for (SupportedProtocol p : collector.getSupportedProtocols()) {
			if (log().isDebugEnabled())
				log().debug(
						"sendInterfaceEvents: sending event for service: "
								+ p.getProtocolName());
		...
		if (collector.hasSnmpCollection()
			...
			for (InetAddress xifaddr : extraTargets.keySet()) {
				...
				// nodeGainedService
				//
				List<SupportedProtocol> supportedProtocols = extraTargets
						.get(xifaddr);
				log().debug(
						"interface " + xifaddr + " supports "
								+ supportedProtocols.size() + " protocols.");
				if (supportedProtocols != null) {
					for (SupportedProtocol p : supportedProtocols) {
						createAndSendNodeGainedServiceEvent(node, xifaddr,
								p.getProtocolName(), null);
					}
				}
			}
	...
	private void createAndSendNodeAddedEvent(DbNodeEntry nodeEntry) {
		...
		bldr.addParam(EventConstants.PARM_NODE_LABEL_SOURCE,
				nodeEntry.getLabelSource());
	...
	private void sendEvent(Event newEvent) {
		...
		try {
			...
			if (log().isDebugEnabled())
				log().debug(
						"sendEvent: successfully sent: " + toString(newEvent));
		} catch (Throwable t) {
			log().warn(
	...
	private String toString(Event e) {
		...
		buf.append(" For ").append(e.getNodeid()).append('/')
				.append(e.getInterface()).append('/').append(e.getService());
		return buf.toString();
	}
	...
	private void createAndSendNodeGainedServiceEvent(DbNodeEntry nodeEntry,
		...
		if (nodeEntry.getSystemName() != null) {
			bldr.addParam(EventConstants.PARM_NODE_SYSNAME,
					nodeEntry.getSystemName());
		}

		// Add sysDescr (if available)
		if (nodeEntry.getSystemDescription() != null) {
			bldr.addParam(EventConstants.PARM_NODE_SYSDESCRIPTION,
					nodeEntry.getSystemDescription());
		}
