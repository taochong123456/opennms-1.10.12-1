...
import org.opennms.netmgt.dao.ProtocolPropertyDao;
...
public class PluginManager implements InitializingBean {
    ...
    private ProtocolPropertyDao m_protocolPropertyDao;
     ...
     */
    public CapsdProtocolInfo[] getProtocolSpecification(InetAddress address, int nodeId) {
        /*
        ...
        PLUGINLOOP: while (pluginIter.hasNext()) {
            ...
            for (ProtocolConfiguration pluginConf : getCapsdConfig().getProtocolConfigurations(plugin)) {
                ...
                Map<String,Object> props = m_protocolPropertyDao.findPropsById(nodeId, plugin.getProtocol());
                if(props.size() == 0){
    ...
    /**
     * Returns the CapsdProtocolInfo for data collect 
     *
     * @param address
     *            The address to get protocol information for.
     * @return The array of protocol information instances for the address.
     */
    public Map<String, Object> getProtocolParameters(InetAddress address, int nodeId, String serviceName) {
    	CapsdProtocolInfo[] protocols = getProtocolSpecification(address, nodeId);
    	for(CapsdProtocolInfo protocol: protocols){
    		if(protocol.getProtocol().equals(serviceName)){
    ...
    public ProtocolPropertyDao getProtocolPropertyDao() {
		return m_protocolPropertyDao;
	}

	public void setProtocolPropertyDao(ProtocolPropertyDao m_protocolPropertyDao) {
		this.m_protocolPropertyDao = m_protocolPropertyDao;
	}
