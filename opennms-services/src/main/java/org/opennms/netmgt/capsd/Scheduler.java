...
final class Scheduler implements Runnable, PausableFiber {
	...
	final class NodeInfo implements Runnable {
		...
		public void run() {
			try {
				m_rescanProcessorFactory.createRescanProcessor(getNodeId())
						.run();
			} finally {
	 ...
	 * 
	 */
	Scheduler(FifoQueue<Runnable> rescanQ,
			RescanProcessorFactory rescanProcessorFactory) throws SQLException {
		...
		if (log().isDebugEnabled())
			log().debug(
					"Scheduler: initial rescan sleep time(millis): "
							+ m_initialSleep);
		...
		NodeInfo smbInfo = new NodeInfo(SMB_REPARENTING_IDENTIFIER,
				lastSmbReparenting, m_interval);
		...
		if (log().isDebugEnabled())
			log().debug(
					"Scheduler: done loading known nodes, node count: "
							+ m_knownNodes.size());
	}
	...
	private void loadKnownNodes() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (log().isDebugEnabled())
					log().debug(
							"loadKnownNodes: retrieved nodeid " + nodeId
									+ ", now getting last poll time.");
				...
				if (rset.next()) {
					...
					if (lastPolled != null && rset.wasNull() == false) {
						if (log().isDebugEnabled())
							log().debug(
									"loadKnownNodes: adding node " + nodeId
											+ " with last poll time "
											+ lastPolled);
						NodeInfo nodeInfo = new NodeInfo(nodeId, lastPolled,
								m_interval);
	...
	void scheduleNode(int nodeId) throws SQLException {
		...
		try {
			db = DataSourceFactory.getInstance().getConnection();
			d.watch(db);
			PreparedStatement ifStmt = db
					.prepareStatement(SQL_GET_LAST_POLL_TIME);
			d.watch(ifStmt);
			...
			if (rset.next()) {
				...
				if (lastPolled != null && rset.wasNull() == false) {
					if (log().isDebugEnabled())
						log().debug(
								"scheduleNode: adding node " + nodeId
										+ " with last poll time " + lastPolled);
					m_knownNodes.add(new NodeInfo(nodeId, lastPolled,
							m_interval));
				}
			} else
				log().warn(
						"scheduleNode: Failed to retrieve last polled time from database for nodeid "
								+ nodeId);
		} finally {
			d.cleanUp();
		}
		...
		synchronized (m_knownNodes) {
			...
			while (iter.hasNext()) {
				...
				if (nodeInfo.getNodeId() == nodeId) {
					log().debug(
							"unscheduleNode: removing node " + nodeId
									+ " from the scheduler.");
	...
	void forceRescan(int nodeId) {
		try {
			m_rescanQ.add(m_rescanProcessorFactory
					.createForcedRescanProcessor(nodeId));
		} catch (FifoQueueException e) {
			log().error(
					"forceRescan: Failed to add node " + nodeId
							+ " to the rescan queue.", e);
			...
			if (firstPass) {
				...
				synchronized (this) {
					try {
						if (log().isDebugEnabled())
							log().debug(
									"Scheduler.run: initial sleep configured for "
											+ m_initialSleep
											+ "ms...sleeping...");
						wait(m_initialSleep);
						...
						if (log().isDebugEnabled())
							log().debug(
									"Scheduler.run: interrupted exception during initial sleep...exiting.");
						break; // exit for loop
					}
				}
			}
			...
			synchronized (m_knownNodes) {
				...
				while (iter.hasNext()) {
					...
					try {
						...
						log().info(
								"Scheduler.schedule: failed to add new node to rescan queue",
								ex);
