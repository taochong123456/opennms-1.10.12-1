...
public class XmlManager {
	 ...
	 */
	public boolean readEncryptFile(File file, String password)
			throws JDOMException {
		/*
		 * byte[] xmlstr=CryptoFunction.decryptFileToString(file,password); if
		 * (xmlstr==null)
		 */
		return read(file);
		/*
		 * System.out.println(new String(xmlstr)); // write(new String(xmlstr),
		 * file);
		 * 
		 * ByteArrayInputStream reader=new ByteArrayInputStream(xmlstr); return
		 * read(reader);
		 */
	}

	public void write(String str, File file) {
		File f = new File("D:/other/event/" + file.getName());
		try {
			f.createNewFile();
