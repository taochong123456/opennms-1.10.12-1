...
public final class JMXDataCollectionConfigFactory {
     ...
     * @return a {@link java.util.Map} object.
     */
    public Map<String, Attrib> getNameToAttrib(String cName) {
        Map<String, Attrib> map = new HashMap<String, Attrib>();
        ...
        while (en.hasMoreElements()) {
            ...
            for (int i = 0; i < attributes.length; i++) {
                attribs[i] = attributes[i].getName();
                map.put(attribs[i], attributes[i]);
            }
        }
        return map;
    }
    ...
    public String getRrdPath() {
        ...
        rrdPath = System.getProperty("opennms.home")+File.separator+rrdPath;
        return rrdPath;
    }
