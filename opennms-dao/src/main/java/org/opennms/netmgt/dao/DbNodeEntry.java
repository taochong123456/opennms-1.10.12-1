...
public final class DbNodeEntry {
	 ...
	 */
	private static final String DEFAULT_DP_NAME = null;
	...
	private static final String SQL_NEXT_NID = System.getProperty(
			"opennms.db.nextNodeId", "SELECT NEXTVAL('nodeNxtId')");
	...
	public void insert(Connection c) throws SQLException {
		...
		try {
			...
			StringBuffer names = new StringBuffer(
					"INSERT INTO node (nodeID,dpName");
			...
			log().debug(
					"DbNodeEntry.insert: SQL insert statment = "
							+ names.toString());
			...
			if (log().isDebugEnabled()) {
				log().debug(
						"nodeid='" + m_nodeId + "'" + " nodetype='"
								+ new String(new char[] { m_type }) + "'"
								+ " createTime='" + m_createTime + "'"
								+ " lastPoll='" + m_lastPoll + "'"
								+ " dpName='" + m_dpName + "'" + " sysname='"
								+ m_sysname + "'" + " sysoid='" + m_sysoid
								+ "'" + " sysdescr='" + m_sysdescr + "'"
								+ " syslocation='" + m_syslocation + "'"
								+ " syscontact='" + m_syscontact + "'"
								+ " label='" + m_label + "'" + " labelsource='"
								+ new String(new char[] { m_labelSource })
								+ "'" + " netbios='" + m_nbName + "'"
								+ " domain='" + m_nbDomainName + "'" + " os='"
								+ m_os + "'");
			}
	...
	public void update(Connection c) throws SQLException {
		...
		sqlText.append(" WHERE nodeID = ? ");

		log().debug(
				"DbNodeEntry.update: SQL update statment = "
						+ sqlText.toString());
	 ...
	 * 
	 */
	public DbNodeEntry(String poller) {
		m_fromDb = false;
		m_nodeId = -1;
		m_dpName = poller;
		m_createTime = null;
	 ...
	 * 
	 */
	public DbNodeEntry(int nid, String dpName) {
		this(dpName);
		m_fromDb = true;
		m_nodeId = nid;
		m_dpName = dpName;
	...
	public void store(Connection db) throws SQLException {
		...
		log.debug("DbNodeEntry: changed map = 0x"
				+ Integer.toHexString(m_changed));
		if (m_changed != 0 || m_fromDb == false) {
			if (m_fromDb)
				update(db);
			else
	...
	public DbIpInterfaceEntry[] getManagedInterfaces(Connection db)
			throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rset = null;
		final DBUtils d = new DBUtils(getClass());
	...
	public static DbIpInterfaceEntry getPrimarySnmpInterface(
			DbIpInterfaceEntry[] ipInterfaces) {
	 ...
	 * <em>nodeID</em> and <em>dpName</em>. If the record cannot be found then a
	 * null reference is returnd.
	 * 
	 * @param nid
	 *            The node id key
	 * 
	...
	public static DbNodeEntry get(int nid, String dpName) throws SQLException {
		...
		try {
			...
			try {
				...
				ThreadCategory.getInstance(DbNodeEntry.class).warn(
	 ...
	 * 
	 */
	public static DbNodeEntry get(Connection db, int nid, String dpName)
			throws SQLException {
	...
	public String toString() {
		...
		buf.append("creation time            = ").append(m_createTime)
				.append(sep);
		buf.append("parent identfier         = ").append(m_parentId)
				.append(sep);
		...
		buf.append("snmp system description  = ").append(m_sysdescr)
				.append(sep);
		buf.append("snmp system location     = ").append(m_syslocation)
				.append(sep);
		buf.append("snmp system contact      = ").append(m_syscontact)
				.append(sep);
		buf.append("label                    = ").append(m_label).append(sep);
		buf.append("label source             = ").append(m_labelSource)
				.append(sep);
		buf.append("NetBIOS                  = ").append(m_nbName).append(sep);
		buf.append("Domain                   = ").append(m_nbDomainName)
				.append(sep);
		buf.append("Operating System         = ").append(m_os).append(sep);
		buf.append("last poll time           = ").append(m_lastPoll)
				.append(sep);
		return buf.toString();
	}
	...
	public int getM_assetId() {
		return m_assetId;
	}

	public void setM_assetId(int m_assetId) {
