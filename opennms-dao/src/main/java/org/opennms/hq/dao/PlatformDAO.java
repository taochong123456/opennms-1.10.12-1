...
public class PlatformDAO extends AbstractDaoHibernate<Platform, Integer> implements OnmsDao<Platform,Integer> {

    private final VirtualDAO virtualDAO;

    public PlatformDAO(VirtualDAO virtualDAO) {
    ...
    public Collection<Platform> findAll_orderName(boolean asc) {
        return getSession().createCriteria(Platform.class).createAlias("resource", "r").addOrder(
    ...
    public Collection<Platform> findAll_orderCTime(boolean asc) {
        return getSession().createCriteria(Platform.class).addOrder(
            asc ? Order.asc("creationTime") : Order.desc("creationTime")).list();
    }

    public Collection<Platform> findByCTime(long ctime) {
        return getSession().createCriteria(Platform.class).add(Restrictions.gt("creationTime", new Long(ctime))).addOrder(
...
*/
    ...
    public List<Platform> findByServers(Integer[] ids) {
        return getSession().createCriteria(Platform.class).createAlias("resource", "r").createAlias(
