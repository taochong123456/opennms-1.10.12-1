...
public class MeasurementTemplateDAO extends
		AbstractDaoHibernate<MeasurementTemplate, Integer> implements
		OnmsDao<MeasurementTemplate, Integer> {
	...
	public MeasurementTemplateDAO(CategoryDAO categoryDAO,
			JdbcTemplate jdbcTemplate) {
		super(MeasurementTemplate.class);
		this.catDAO = categoryDAO;
		this.jdbcTemplate = jdbcTemplate;
	}

	MeasurementTemplate create(String name, String alias, String units,
			int collectionType, boolean defaultOn, long defaultInterval,
			boolean designate, String template,
			MonitorableType monitorableType, Category cat, String plugin) {
		MeasurementTemplate mt = new MeasurementTemplate(name, alias, units,
				collectionType, defaultOn, defaultInterval, designate,
				template, monitorableType, cat, plugin);
		save(mt);
		return mt;
	}
	...
	public List<MeasurementTemplate> findAllTemplates(PageInfo pInfo,
			Boolean defaultOn) {
		...
		sql += " order by "
				+ ((MeasurementTemplateSortField) pInfo.getSort())
						.getSortString("t");
	...
	@SuppressWarnings("unchecked")
	public List<MeasurementTemplate> findTemplates(Integer[] ids) {
		return null;
		/*
		 * if (ids.length == 1) { MeasurementTemplate res = get(ids[0]);
		 * 
		 * if (res == null) { return new ArrayList<MeasurementTemplate>(); }
		 * 
		 * return Collections.singletonList(res); }
		 * 
		 * return createCriteria().add(Restrictions.in("id",
		 * ids)).setCacheable(true).setCacheRegion(
		 * "MeasurementTemplate.findTemplates").list();
		 */}

	public List<MeasurementTemplate> findTemplatesByMonitorableType(String type) {
		PageInfo pInfo = PageInfo.getAll(
				MeasurementTemplateSortField.TEMPLATE_NAME, true);
		return findTemplatesByMonitorableType(pInfo, type, null);
	}

	@SuppressWarnings("unchecked")
	public List<MeasurementTemplate> findTemplatesByMonitorableType(
			PageInfo pInfo, String type, Boolean defaultOn) {
		String sql = "select t from MeasurementTemplate t "
	...
	@SuppressWarnings("unchecked")
	public List<MeasurementTemplate> findTemplatesByMonitorableTypeAndCategory(
			String type, String cat) {
		String sql = "select t from MeasurementTemplate t "
				+ "where t.monitorableType.name=? " + "and t.category.name=? "
				+ "order by t.name";

		return getSession().createQuery(sql).setString(0, type)
	...
	List<MeasurementTemplate> findTemplatesByName(List<String> tmpNames) {
		if (tmpNames == null || tmpNames.size() == 0) {
			throw new IllegalArgumentException(
					"no template names passed to MeasurementTemplateImpl.findTemplatesByName()");
		}
		StringBuilder sql = new StringBuilder()
				.append("select t from MeasurementTemplate t where");
		...
		if (getTmpQuery == null) {
			throw new HibernateException(
					"failed creating template retrieval by name query");
		}
		int i = 0;
	...
	@SuppressWarnings("unchecked")
	public List<MeasurementTemplate> findDefaultsByMonitorableType(String mt,
			int appdefType) {
		String sql = "select t from MeasurementTemplate t "
				+ "join fetch t.monitorableType mt "
				+ "where mt.name=? and mt.appdefType=? "
				+ "and t.defaultOn = true " + "order by mt.name";

		return getSession().createQuery(sql).setString(0, mt)
	...
	List<MeasurementTemplate> findDesignatedByMonitorableType(String mt,
		...
		String sql = "select t from MeasurementTemplate t "
				+ "join fetch t.monitorableType mt "
				+ "where mt.name=? and mt.appdefType=? "
				+ "and t.designate = true " + "order by mt.name";
	...
	List<MeasurementTemplate> findRawByMonitorableType(MonitorableType mt) {
		...
		List<Integer> tids = getSession().createQuery(sql).setParameter(0, mt)
				.list();
		List<MeasurementTemplate> rtn = new ArrayList<MeasurementTemplate>(
				tids.size());
	...
	public List<MeasurementTemplate> findDerivedByMonitorableType(String name) {
		...
		//
		String sql = "select m from MeasurementTemplate m "
				+ "join fetch m.monitorableType mt " + "where mt.name = ? "
				+ "order by m.name asc ";
	...
	public void createTemplates(final String pluginName,
			final Map<MonitorableType, List<MonitorableMeasurementInfo>> toAdd) {/*
																				 ...
																				 * warn
																				 * (
																				 * "ALIAS field of EAM_MEASUREMENT_TEMPLATE truncated: original value was "
																				 * +
																				 * info
																				 * .
																				 * getAlias
