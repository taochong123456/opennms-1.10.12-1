...
extends AbstractDaoHibernate<Action, Integer> implements OnmsDao<Action,Integer>{
    ...
    void deleteAlertDefinition(AlertDefinition def) {
        // Find all actions
        List<Action> actions = (List<Action>) getSession().createCriteria(Action.class).add(
        ...
        Query q = getSession().createQuery(sql).setParameter("def", def);
