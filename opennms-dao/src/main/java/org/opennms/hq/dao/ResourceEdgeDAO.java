...
public class ResourceEdgeDAO extends
		AbstractDaoHibernate<ResourceEdge, Integer> implements
		OnmsDao<ResourceEdge, Integer> {
	public ResourceEdgeDAO() {
		super(ResourceEdge.class);
	}

	public ResourceEdge create(Resource from, Resource to, int distance,
	...
	Map findDescendantMap(Resource r) {
		String sql = "select e.to, e.distance from ResourceEdge e "
				+ "where e.from = :from " + "and distance > :distance";

		List vals = getSession().createQuery(sql).setParameter("from", r)
				.setInteger("distance", 0).list();
	...
	public Collection<ResourceEdge> findChildEdges(List<Resource> resources,
			ResourceRelation rel) {
		String sql = "from ResourceEdge e " + "where e.from in (:resources) "
				+ "and distance = 1 " + "and rel_id = :rel_id ";
		final List<ResourceEdge> rtn = new ArrayList<ResourceEdge>(
				resources.size());
		final Query query = getSession().createQuery(sql);
		int size = resources.size();
		for (int i = 0; i < size; i += BATCH_SIZE) {
			int end = Math.min(i + BATCH_SIZE, size);
			final List<ResourceEdge> edges = query
					.setParameterList("resources", resources.subList(i, end))
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Collection<ResourceEdge> findDescendantEdges(
			List<Resource> resources, ResourceRelation rel) {
		String sql = "from ResourceEdge e " + "where e.from in (:resources) "
	 ...
	 * @return {@link Collection} of {@link ResourceEdge}s
	 */
	public Collection<ResourceEdge> findDescendantEdges(Resource r,
			ResourceRelation rel) {
		return findDescendantEdges(Collections.singletonList(r), rel);
	}

	public List<ResourceEdge> findDescendantEdgesByNetworkRelation(
			Integer resourceId, List<Integer> platformTypeIds,
			String platformName) {/*
								 * String nameEx = null; String sql =
								 * "select {e.*} from EAM_RESOURCE_EDGE e " +
								 * " join EAM_RESOURCE r on e.to_id = r.id " +
								 * " join EAM_PLATFORM p on p.resource_id = r.id "
								 * + " where e.rel_id = " +
								 * AuthzConstants.RELATION_NETWORK_ID +
								 * " and e.distance != 0 ";
								 * 
								 * if (resourceId != null) { sql +=
								 * " and r.id = :rid "; } if (platformTypeIds !=
								 * null && !platformTypeIds.isEmpty()) { sql +=
								 * " and p.platform_type_id in (:ptids) "; }
								 * 
								 * if (platformName != null &&
								 * platformName.trim().length() > 0) { HQDialect
								 * dialect = getHQDialect(); nameEx =
								 * dialect.getRegExSQL("r.sort_name", ":regex",
								 * true, false);
								 * 
								 * sql += " and (" + nameEx + ") "; } sql +=
								 ...
								 * 
								 * if (resourceId != null) {
								 * query.setInteger("rid",
								 * resourceId.intValue()); } if (platformTypeIds
								 * != null && !platformTypeIds.isEmpty()) {
								 * query.setParameterList("ptids",
								 * platformTypeIds, new IntegerType()); }
								 * 
								 * if (nameEx != null) {
	...
	@SuppressWarnings("unchecked")
	public Collection<ResourceEdge> findAncestorEdges(Resource r,
			ResourceRelation rel) {
		String sql = "from ResourceEdge e where e.from = :from and distance < :distance and rel_id = :rel_id ";
		return getSession().createQuery(sql).setParameter("from", r)
				.setInteger("distance", 0)
				.setInteger("rel_id", rel.getId().intValue()).list();
	}

	public void deleteEdges(Resource r) {
		String sql = "from ResourceEdge where to_id = :to or from_id = :from)";
		@SuppressWarnings("unchecked")
		List<ResourceEdge> edges = getSession().createQuery(sql)
				.setParameter("to", r).setParameter("from", r).list();
		for (ResourceEdge edge : edges) {
			delete(edge);
		}
	}

	public void deleteEdges(Resource r, ResourceRelation rel) {
		String sql = "from ResourceEdge where (to_id = :to or from_id = :from) and rel_id = :rel_id ";
		@SuppressWarnings("unchecked")
		List<ResourceEdge> edges = getSession().createQuery(sql)
				.setParameter("to", r).setParameter("from", r)
				.setInteger("rel_id", rel.getId()).list();
		for (ResourceEdge edge : edges) {
			delete(edge);
		}
	}

	public void deleteEdge(Resource parent, Resource child, ResourceRelation rel) {
		String sql = "from ResourceEdge where from_id = :from and to_id = :to and rel_id = :rel_id ";
		@SuppressWarnings("unchecked")
		List<ResourceEdge> edges = getSession().createQuery(sql)
				.setParameter("from", parent).setParameter("to", child)
				.setInteger("rel_id", rel.getId()).list();
		for (ResourceEdge edge : edges) {
			delete(edge);
		}
	...
	@SuppressWarnings("unchecked")
	public Collection<ResourceEdge> getDescendantEdgesOfType(
			Collection<Resource> resources, Resource prototype,
			ResourceRelation relation, int distance) {
		String sign = (distance < 0) ? "<=" : ">=";
		distance = Math.abs(distance);
		final StringBuilder hqlbuf = new StringBuilder(64)
				.append("FROM ResourceEdge re ").append("WHERE distance ")
				.append(sign).append(" :dist and relation=:relation ")
	...
	// null
	public ResourceEdge getParentEdge(Resource resource,
			ResourceRelation relation) {
	...
	public List<ResourceEdge> findChildEdges(Resource resource,
			ResourceRelation relation) {
		String hql = "from ResourceEdge re "
				+ "where re.from=:from and distance=1 and relation=:relation";

		return getSession().createQuery(hql).setParameter("from", resource)
				.setParameter("relation", relation).list();
	}

	public List<ResourceEdge> findByName(String name, ResourceRelation relation) {
		return getSession().createCriteria(ResourceEdge.class)
				...
				.add(Restrictions.eq("distance", new Integer(0))).list();
	}

	// Returns the number of descendants of a given resource
	public int getDescendantCount(Resource resource, ResourceRelation relation) {
		...
		return ((Number) getSession().createQuery(hql)
				.setParameter("from", resource)
				.setParameter("relation", relation).iterate().next())
				.intValue();
	}

	// Checks whether or not a given resource has a value to=from
	public boolean hasSelfEdge(Resource resource) {
		String sql = "from ResourceEdge where from_id = :from and from_id = to_id";
		@SuppressWarnings("unchecked")
		int size = getSession().createQuery(sql)
				.setInteger("from", resource.getId()).list().size();
		if (size > 0) {
			return true;
		}
		return false;
	...
	public boolean hasChildren(Resource parent, ResourceRelation relation) {
	...
	private boolean checkEdgeExistence(Resource resource,
			ResourceRelation relation, int distance) {
		String hql = "select count(re) from ResourceEdge re "
				+ "where re.from=:from and distance=:distance and relation=:relation";

		int result = ((Number) getSession().createQuery(hql)
				.setParameter("from", resource)
				.setParameter("relation", relation)
				.setInteger("distance", distance).iterate().next()).intValue();
	...
	public boolean isResourceChildOf(Resource parent, Resource child) {
		...
		List results = getSession()
				...
				.setInteger("rel_id",
						AuthzConstants.RELATION_CONTAINMENT_ID.intValue())
				.list();
