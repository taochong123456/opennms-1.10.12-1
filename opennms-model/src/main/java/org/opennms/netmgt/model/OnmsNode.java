...
public class OnmsNode extends OnmsEntity implements Serializable,
	 ...
	 * chassis that should be reflected as a subcomponent or "child", this field
	 * reflects the nodeID of the chassis/physical node/"parent" device.
	 * 
	...
	/**
	 * Flag indicating status of node - 'A' - active - 'D' - deleted
	 * 
	...
	/**
	 * Flag indicating source of nodeLabel - 'U' = user defined - 'H' = IP
	 * hostname - 'S' = sysName - 'A' = IP address
	 * 
	...
	public void setMac(String m_mac) {
		this.m_mac = m_mac;
	}
	...
	public OnmsIpInterface getIpInterfaceByIpAddress(String ipAddress) {
		return getIpInterfaceByIpAddress(InetAddressUtils
	...
	public void mergeNodeAttributes(OnmsNode scannedNode,
		...
		if (hasNewValue(scannedNode.getLabel(), getLabel())) {
			...
			EventBuilder bldr = new EventBuilder(
					EventConstants.NODE_LABEL_CHANGED_EVENT_UEI,
			...
			if (getLabel() != null) {
				...
				bldr.addParam(EventConstants.PARM_OLD_NODE_LABEL_SOURCE,
						getLabelSource());
			}

			if (scannedNode.getLabel() != null) {
				bldr.addParam(EventConstants.PARM_NEW_NODE_LABEL,
						scannedNode.getLabel());
				bldr.addParam(EventConstants.PARM_NEW_NODE_LABEL_SOURCE,
						scannedNode.getLabelSource());
			}
	...
	public void mergeSnmpInterfaces(OnmsNode scannedNode, boolean deleteMissing) {
		...
		for (Iterator<OnmsSnmpInterface> it = getSnmpInterfaces().iterator(); it
				.hasNext();) {

			OnmsSnmpInterface iface = it.next();
			OnmsSnmpInterface imported = scannedInterfaceMap.get(iface
					.getIfIndex());
	...
	public void mergeIpInterfaces(OnmsNode scannedNode,
			EventForwarder eventForwarder, boolean deleteMissing) {
		...
		for (Iterator<OnmsIpInterface> it = getIpInterfaces().iterator(); it
				.hasNext();) {
			OnmsIpInterface dbIface = it.next();
			// find the corresponding scanned Interface
			OnmsIpInterface scannedIface = ipInterfaceMap.get(dbIface
					.getIpAddress());
			...
			if (scannedIface == null) {
				...
				dbIface.mergeInterface(scannedIface, eventForwarder,
						deleteMissing);
				if (scannedPrimaryIf != null && dbIface.isPrimary()
						&& scannedPrimaryIf != scannedIface) {
					dbIface.setIsSnmpPrimary(PrimaryType.SECONDARY);
					oldPrimaryInterface = dbIface;
				}
			}
		...
		for (OnmsIpInterface iface : ipInterfaceMap.values()) {
			...
			if (iface.getIfIndex() != null) {
				iface.setSnmpInterface(getSnmpInterfaceWithIfIndex(iface
						.getIfIndex()));
			}
