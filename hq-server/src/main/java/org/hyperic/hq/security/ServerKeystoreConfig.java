...
@Component
public class ServerKeystoreConfig extends KeystoreConfig {
	private static final Log log = LogFactory
			.getLog(ServerKeystoreConfig.class);

	@Autowired
	public ServerKeystoreConfig(
			@Value("#{securityProperties['server.keystore.path']}") String keystore,
			@Value("#{securityProperties['server.keystore.password']}") String keypass)
			...
			if (!keystoreFile.isAbsolute()) {
				...
				if (basePath.length() == 0) {
					throw new ConfigPropertyException(
							"Cannot determine base path using catalina.base");
				}
			...
			if (!keystoreFile.exists()) {
				throw new ConfigPropertyException(
						"The keystore path ["
								...
								+ "] does not exist. If setting a relative path, it must be relative to the server's hq-server directory.");
			}

			super.setFilePath(keystoreFile.getPath());
			super.setFilePassword(keypass);
			// The server should never generate the keystore. It should already
			// have a keystore running.
			// As a result, it's "custom" setting.
			super.setHqDefault(true);
			super.setKeyCN("Hyperic Server");
		} else {
			throw new ConfigPropertyException(
					"Keystore path (server.keystore.path) is empty");
		}
	}
}
